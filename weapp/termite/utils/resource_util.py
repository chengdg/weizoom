...
#===============================================================================
# get_termite_dialogs ：获得web页面的dialog的资源
#===============================================================================
def get_termite_dialogs():
	#搜集所有的可能包含js/dialog的目录
	dirs = []
	for url_path, dialog_dir_path in settings.TERMITE_WEB_DIALOG_DIRS:
		if dialog_dir_path.endswith('*'):
			dialog_dir_path = dialog_dir_path[:-2]
			for f in os.listdir(dialog_dir_path):
				if os.path.isdir(os.path.join(dialog_dir_path, f)):
					dirs.append({
						'module_parent': dialog_dir_path,
						'module': f,
						'url_path': url_path
					})
		else:
...
#===============================================================================
# get_termite_views ：获得termite的view的资源
#===============================================================================
def get_termite_views():
	dirs = []
	for url_path, weapp_view_dir_path in settings.WEAPP_WEB_VIEW_DIRS:
...
#===============================================================================
# get_termite_models ：获得web页面的model的资源
#===============================================================================
def get_termite_models():
	dirs = []
	for url_path, weapp_view_dir_path in settings.WEAPP_WEB_MODEL_DIRS:
		if weapp_view_dir_path.endswith('*'):
			weapp_view_dir_path = weapp_view_dir_path[:-2]
			for f in os.listdir(weapp_view_dir_path):
				if os.path.isdir(os.path.join(weapp_view_dir_path, f)):
