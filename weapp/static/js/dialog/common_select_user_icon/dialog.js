...
W.dialog.common.SelectUserIconDialog = W.dialog.Dialog.extend({
    events: _.extend({
        'click .selectIconDialog_iconBox': 'onClickIcon',
        'click .tx_toggleIconMenu': 'toggleIcoMenu'
    }, W.dialog.Dialog.prototype.events),

    getTemplate: function() {
        $('#common-select-user-icon-dialog-tmpl-src').template('common-select-user-icon-dialog-tmpl-tmpl');
        return "common-select-user-icon-dialog-tmpl-tmpl";
    },

    getOneIconTemplate: function() {
        $('#common-select-user-icon-dialog-one-icon-tmpl-src').template('common-select-user-icon-dialog-one-icon-tmpl');
        return "common-select-user-icon-dialog-one-icon-tmpl";
    },
            ...
            var task = new W.DelayedTask(function() {
                W.getApi().call({
                    ...
                    success: function(data) {
                        ...
                        _this.$('.selectIconDialog_body_loading').hide();
                    },
                    error: function(resp) {
                        alert('获取图片失败');
                    }
    ...
    onFinishUpload: function(path) {
        ...
        var event = {currentTarget: $icoContent.find('.selectIconDialog_iconBox').last().get(0)};
    ...
    onClickIcon: function(event) {
        ...
        imgBox.find('.selectIconDialog_iconBox_cover').addClass('activeImg').show();
    },
