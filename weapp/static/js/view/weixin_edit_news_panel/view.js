...
W.view.weixin.EditNewsPanel = Backbone.View.extend({
	...
	initialize: function(options) {
        ...
        if (enableSummary) {
            ...
            this.$el.find('#summary').attr('data-validate','cannot-contains,required');
	        this.$('#urlDiv').show();
		...
		this.phone.bind('start-create-news', function() {
			...
			var message = W.model.weixin.Message.createNewsMessage({autoSelect: true});
        ...
        for (var i = 0; i < newsCount; ++i) {
            ...
            console.log(2000000, news, newsMessage)
        ...
        if (newsCount == 0 && options.mode) {
        	if (options.mode == 'single-news') {
        		this.newsEditor.preserveNewsUnder(1);
        		var message = W.model.weixin.Message.createNewsMessage({autoSelect: true, scheduledDate: this.scheduledDate});
        		this.phone.addNews(message);
        	} else if(options.mode == 'multi-news') {
        		this.newsEditor.preserveNewsUnder(2);
        		var messages = [
        			W.model.weixin.Message.createNewsMessage({autoSelect: true})
        		];
	...
	isAllRightNewses: function(newses){
		...
		console.log(isAllRight, 'isAllRightNewses');
		return isAllRight
	}
});