...
class Integral(resource.Resource):
    ...
    def api_post(request):
        ...
        if Member.objects.filter(webapp_id=webapp_id, id=member_id).count() == 0:
            ...
            zeus_req('put', {
                'resource': 'member.member_integral',
                'data': {
                    'member_id': member_id,
                    'integral': integral,
                }
            })            
            zeus_resp = zeus_req('post', {
                'resource': 'member.member_integral_log',
                'data': {
                    'member_id': member_id,
                    'integral': integral,
                    'reason': reason
        ...
        response = create_response(200)
        response.data = {
            'integral': zeus_resp['integral'] #Member.objects.get(webapp_id=webapp_id, id=member_id).integral
        }
        return response.get_response()
