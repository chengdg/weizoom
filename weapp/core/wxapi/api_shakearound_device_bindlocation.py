...
COMPONENT_URI = 'shakearound/device/bindlocation'
class WeixinShakeAroundDeviceUpdate(object):
	""""
	http请求方式: POST（请使用https协议）
	https://api.weixin.qq.com/shakearound/device/bindlocation?access_token=ACCESS_TOKEN
	POST数据格式：json
	POST数据例子：
	{
	     "device_identifier":{
	...
	def parese_post_param_json_str(self, args):
		...
		return '{"quantity": {"device_id": %d}, "poi_id": %d}' % (args[0], args[1])
