...
COMPONENT_URI = 'cgi-bin/component/api_get_authorizer_info'
class WeixinGetAuthorizerInfo(object):

	def get_get_request_url_and_api_info(self, component_access_token=None, varargs=()):
		if len(varargs) == 0:
			raise ValueError(u'WeixinGetAuthorizerInfo.get_get_request_url error, param illegal')
	...
	def parese_post_param_json_str(self, args):
		if len(args) == 0 or len(args) > 3:
			raise ValueError(u'WeixinGetAuthorizerInfo param number illegal')

		return '{"component_appid": "%s", "authorizer_appid": "%s"}' % (args[0], args[1])
