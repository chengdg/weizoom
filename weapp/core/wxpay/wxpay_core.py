...
# 财付通支付参数信息中的空值和签名参数
#========================================================
def filter_wxpay_request_params_to_build_sign(params):
	assert_param(params)

	# 首先除去空值和签名参数
	filtered_params = {}
	for key in params.keys():
		if params[key] == '' or None == params[key] or WxpayRequestParams.SIGN_TYPE == key:
			continue
...
def build_str_params(params):
	...
	for key, val in params.iteritems():
		if isinstance(val, unicode):
			val = val.encode(WxpayConfig.input_charset)
		str_params[key] = val
