	...
	def get(args):
		...
		if product_ids:
			...
			if order_has_products.count()>0:
				...
				orders_list = mall_models.Order.objects.filter(id__in=order_dot_ids,status__in=[3,4,5]).exclude(order_id__icontains="s").values('payment_time').annotate(today_order_num=Count('payment_time'))
