...
W.view.card.channelFilter = Backbone.View.extend({
    events: {
    ...
    getFilterValue: function(){
        ...
        if (channelName.length > 0){
            dataValue.push('channel:'+channelName);
        }
        //money
        if (lowMoney.length > 0 && highMoney.length > 0) {
    ...
    getTemplate: function() {
        $('#card-channel-filter-view-tmpl-src').template('card-channel-filter-view-tmpl');
        return 'card-channel-filter-view-tmpl';
    },
