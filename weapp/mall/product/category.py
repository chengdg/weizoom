...
class Category(resource.Resource):
	...
	def api_post(request):
		...
		if 0 != product_categorys.count():
			...
			name=request.POST.get('name', '').strip(),
			product_count=len(product_ids) + int(product_categorys.first().product_count)
			)
		else:
			response = create_response(500)
	...
	def api_delete(request):
		...
		if product_id:
			...
			product_category = mall_models.ProductCategory.objects.filter(owner=request.manager, id=category_id)
			product_category.update(
				product_count=int(product_category.first().product_count) - 1
			)			
		elif category_id:
