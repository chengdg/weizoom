...
def pre_delete_product_model_property_handler(model_property, request, **kwargs):
    try:
        ...
        # 获取仍然有规格的商品id
        ok_product_ids = product_models = set(ProductModel.objects.filter(
            product_id__in=product_ids, is_deleted=False
        ).values_list('product_id', flat=True))
        need_update_stock = product_ids - ok_product_ids
        ProductModel.objects.filter(product_id__in=need_update_stock, is_standard=True).update(
            is_deleted=False, stock_type=PRODUCT_STOCK_TYPE_LIMIT, stocks=0)
        #from module_api import remove_product_from_all_shopping_cart
        #remove_product_from_all_shopping_cart(product_id=product_id)
    except:
        if settings.DEBUG:
    ...
    if len(card_passes) > 0:
        ...
        for card_name in card_names:
            ...
            if len(card_passes) > card_index and len(card_name) > 0:
                msg, weizoom_card = weizoom_card_api.check_weizoom_card(card_name, card_passes[card_index],request.webapp_user,request.member,request.webapp_owner_id,request.user_profile.webapp_id)
                if msg:
                    fail_msg['data']['msg'] = msg
                    return fail_msg
                else:
...
def products_not_online_handler_for_promotions(product_ids, request, shelve_type=None,**kwargs):
    ...
    for promotion in promotion_models.Promotion.objects.filter(id__in=promotionIds).filter(~Q(status = promotion_models.PROMOTION_STATUS_DELETED)):
        ...
        # if promotion.type != promotion_models.PROMOTION_TYPE_COUPON:
        #     if promotion == promotion_models.PROMOTION_TYPE_INTEGRAL_SALE and shelve_type == 'delete':
        #         # 处理积分应用
        #         not_deleted_promotion_product_count = promotion_models.ProductHasPromotion.objects.filter(
        #             promotion=promotion,
        #             product__is_deleted=False).count()
        ...
        #                                                                                           product__is_deleted=False).count()
        #     if not_deleted_promotion_product_count == 0:  # not_deleted_promotion_product_count 该promotion未删除的商品数
        #         promotion.status = promotion_models.PROMOTION_STATUS_DISABLE
        #         promotion.save()
