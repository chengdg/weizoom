	...
	try:
		...
		if member_qrcode:
			...
			if member and member.is_new and MemberQrcodeLog.objects.filter(member_id=member.id).count() == 0:
				...
				if member_qrcode.owner.username in ['ceshi01', 'kftengyi'] and TengyiMember.objects.filter(member_id=member_qrcode.member_id).first(): #判断二维码拥有者是否星级会员
					...
					if TengyiMemberRelation.objects.filter(member_id=member.id).count() <= 0:
						TengyiMemberRelation.objects.create(
							member_id=member.id,
							recommend_by_member_id=member_qrcode.member_id,
						)
