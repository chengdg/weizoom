...
(function($, undefined) {
    ...
    ScratchCard.prototype = {
        ...
        touchmove: function(event){            
            ...
            var lastX = this.lastX || mouseX;
            var lastY = this.lastY || mouseY;
            var ctx = this.ctx;
            if(this.isMouseDown){
                ctx.lineJoin = "miter";
                ctx.globalCompositeOperation = "destination-out";
                ctx.arc(this.lastX, this.lastY, 10, 0, 2*Math.PI, false);
                ctx.fill();
                this.lastX = mouseX;
                this.lastY = mouseY;
            }
    ...
    $.widget("mobile.scratchCard", $.mobile.widget, {
        ...
        setting: {
            ...
            scratchRank: function(_this) {
                return _this.$el.attr('scratch-rank') || '';
            },
            //是否已刮过
...
//                        data = data || {};
	                    _this.$el.find('.tx_rank_lay').text(prize?prize.name:_this.setting.not_win_desc(_this));
                        _this.$el.trigger('complate', {integral: data.member.integral});
                    },
                    error: function(resp) {
						_this.$el.find('.tx_rank_lay').text(_this.setting.not_win_desc(_this));
                    }
                });
            });
        },
        ...
        _buildTemplate: function() {
            ...
            setTimeout(function() {
                _this.canvasView = new ScratchCard({
                    ...
                    textContent: scratchRank
                });
                _this.$el.find('.tx_rank_lay').text(scratchRank);
