...
class Command(BaseCommand):
	...
	def handle(self,*args, **options):
			...
			for c in City.objects.all():
				orders = Order.objects.filter(webapp_id__in=webapp_ids, origin_order_id__lte=0, status__in=[3,4,5], payment_time__gte= first_datetime, payment_time__lte=datetime.now(), area__startswith="%s_%s" % (c.province_id, c.id))
				...
				total_orders = Order.objects.filter(webapp_id__in=webapp_ids, origin_order_id__lte=0, status__in=[2,3,4,5], area__startswith="%s_%s" % (c.province_id, c.id))
