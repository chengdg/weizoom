...
class Command(BaseCommand):
	...
	def handle(self, **options):
		...
		for tengyi_member_sycle in all_tengyi_member_rebate_sycle:
			...
			print consumer_rebates, money_sum
		...
		for tengyi_member_sycle in all_tengyi_member_rebate_sycle:
			...
			print all_recommend_member_ids,'-----'
			#获取在账期内所有的获得购物返利的推广会员
			start_at = tengyi_member_sycle.start_time.date()
			end_at = yesterday
			all_recommend_member_rebate_sycle = TengyiMemberRebateCycle.objects.filter(member_id__in=all_recommend_member_ids, receive_reward_at__range=(start_at, end_at), is_recommend_member_receive_reward=False)
			print all_recommend_member_rebate_sycle
			#返利
