...
class Command(BaseCommand):
	...
	def handle(self, today_str='today', **options):
		...
		for tengyi_member_sycle in all_tengyi_member_rebate_sycle:
			...
			orders = Order.objects.filter(webapp_user_id=cur_webapp_user_id, created_at__range=(start_at, end_at), status__in=[3,4,5], origin_order_id__lte=0, is_first_order=False,payment_time__lt=seven_days_ago_date)
