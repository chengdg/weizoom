...
def step_impl(context,webapp_user_name,webapp_owner_name,survey_name):
	...
	response = context.client.post('/m/apps/survey/api/survey_prize/?_method=put', params)
	context.survey_result = json.loads(response.content)

# @when(u'{webapp_user_name}参加微信抽奖活动"{survey_name}"于"{date}"')
# def step_impl(context,webapp_user_name,survey_name,date):
# 	context.execute_steps(u"when %s参加微信抽奖活动'%s'" % (webapp_user_name, survey_name))
# 	survey_id = __get_survey_id(survey_name)
# 	date = bdd_util.get_date(date)
# 	surveyparticipance = surveyParticipance.objects.get(member_id=context.member.id,belong_to=str(survey_id))
# 	surveyparticipance.update(set__survey_date=date)
# 	surveyparticipance.save()
#
...
#
# @When(u"{webapp_user_name}把{webapp_owner_name}的微信抽奖活动'{survey_name}'的活动链接分享到朋友圈")
# def step_impl(context, webapp_user_name, webapp_owner_name,survey_name):
# 	survey_id = __get_survey_id(survey_name)
# 	webapp_owner_id = context.webapp_owner_id
# 	url = '/m/apps/survey/m_survey/?webapp_owner_id=%s&id=%s' % (webapp_owner_id, survey_id)
# 	url = bdd_util.nginx(url)
