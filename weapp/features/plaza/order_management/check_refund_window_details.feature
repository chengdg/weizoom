#_author_:张三香 2016.09.21

Feature:查看自营平台退款录入弹窗详情
	"""
	1、对某个子订单点击【申请退款】，弹出退款录入的界面
	2、退款录入界面显示信息:
		a.弹框上方显示：退款录入，当前订单应退￥x.xx（x.xx=商品总金额（商品售价x商品数量）+运费）
		b.展示母订单支付金额详情，格式'母订单支付金额：现金￥x.xx+微众卡￥x.xx+优惠券￥x.xx+积分抵扣￥x.xx=￥x.xx'
		c.当母订单中已经有子订单退款录入时，再对其他子订单操作退款时，添加【已录入退款金额】详情）；录入时，现金、微众卡可退金额需要扣除“已退款金额”的相应部分；
		d.运营人员需要输入当前子订单退款的各个方式对应的金额；其中，现金、微众卡支付金额不得大于母订单支付时的对应金额；
		e.录入的'现金+微众卡+优惠券+积分抵扣'的总和应等于'商品总金额+运费'
		f.积分比例按照系统当前的抵扣比例展示；输入积分数后即按照该比例展示抵扣的金额；
		g.现金、微众卡输入的金额大于最多可退金额时，系统提示“最多可退x元”；
		h.'共计'需要根据已填写的金额实时计算变化;当共计金额与应退金额一致时,才可以提交退款申请;否则点击提交后提示"退款金额不等于x"

	"""

Background:
	Given 重置'weizoom_card'的bdd环境
	Given 重置'apiserver'的bdd环境
	Given zy1登录系统
	And zy1设定会员积分策略
		"""
		{
			"integral_each_yuan":2
		}
		"""
	And zy1已有微众卡支付权限
	And zy1已添加支付方式
		"""
		[{
			"type":"货到付款"
		},{
			"type":"微信支付"
		},{
			"type":"支付宝"
		},{
			"type":"微众卡支付"
		}]
		"""

	#创建微众卡
		Given test登录管理系统::weizoom_card
		When test新建通用卡::weizoom_card
			"""
			[{
				"name":"10元微众卡",
				"prefix_value":"100",
				"type":"virtual",
				"money":"10.00",
				"num":"2",
				"comments":"微众卡"
			}]
			"""
		#微众卡审批出库
		When test下订单::weizoom_card
			"""
			[{
				"card_info":[{
					"name":"10元微众卡",
					"order_num":"2",
					"start_date":"2016-04-07 00:00",
					"end_date":"2019-10-07 00:00"
				}],
				"order_info":{
					"order_id":"0001"
					}
			}]
			"""
		#激活微众
		When test激活卡号'100000001'的卡::weizoom_card
	#创建供货商、设置商家运费、同步商品到自营平台
		#创建供货商
			Given 创建一个特殊的供货商，就是专门针对商品池供货商
				"""
				{
					"supplier_name":"商家1"
				}
				"""
			Given 创建一个特殊的供货商，就是专门针对商品池供货商
				"""
				{
					"supplier_name":"商家2"
				}
				"""
			Given 创建一个特殊的供货商，就是专门针对商品池供货商
				"""
				{
					"supplier_name":"商家3"
				}
				"""
		#设置商家运费
			#商家1设置运费-满50包邮，否则收取统一运费1元
			when 给供货商添加运费配置
				"""
				{
					"supplier_name": "商家1",
					"postage":1,
					"condition_money": "50"
				}
				"""
		#同步商品到自营平台
			Given 给自营平台同步商品
				"""
				{
					"accounts":["zy1"],
					"supplier_name":"商家1",
					"name": "商品1a",
					"promotion_title": "商品1a促销",
					"purchase_price": 9.00,
					"price": 10.00,
					"weight": 1,
					"image": "love.png",
					"stocks": 100,
					"detail": "商品1a描述信息"
				}
				"""
			Given 给自营平台同步商品
				"""
				{
					"accounts":["zy1"],
					"supplier_name":"商家1",
					"name": "商品1b",
					"promotion_title": "商品1b促销",
					"purchase_price": 19.00,
					"price": 20.00,
					"weight": 1,
					"image": "love.png",
					"stocks": 100,
					"detail": "商品1b描述信息"
				}
				"""
			Given 给自营平台同步商品
				"""
				{
					"accounts":["zy1"],
					"supplier_name":"商家2",
					"name": "商品2a",
					"promotion_title": "商品2a促销",
					"purchase_price": 9.00,
					"price": 10.00,
					"weight": 1,
					"image": "love.png",
					"stocks": 100,
					"detail": "商品2a描述信息"
				}
				"""
			Given 给自营平台同步商品
				"""
				{
					"accounts":["zy1"],
					"supplier_name":"商家3",
					"name": "商品3a",
					"promotion_title": "商品3a促销",
					"purchase_price": 9.00,
					"price": 10.00,
					"weight": 1,
					"image": "love.png",
					"stocks": 100,
					"detail": "商品3a描述信息"
				}
				"""
	#自营平台从商品池上架商品
		Given zy1登录系统
		When zy1上架商品池商品"商品1a"
		When zy1上架商品池商品"商品1b"
		When zy1上架商品池商品"商品2a"
		When zy1上架商品池商品"商品3a"
	#创建优惠券-全体券10元
		When zy1添加优惠券规则
			"""
			[{
				"name": "全店通用券1",
				"money": 10.00,
				"limit_counts": "无限",
				"count": 5,
				"start_date": "2013-10-10",
				"end_date": "10天后",
				"description":"使用说明",
				"coupon_id_prefix": "coupon1_id_"
			}]
			"""
	When bill关注zy1的公众号::apiserver


	#bill购买多个供货商的商品
		When bill关注zy1的公众号::apiserver
		When bill访问zy1的webapp::apiserver
		#10101-微信支付（商品1a,2+商品1b）-现金21+微众卡10+优惠券10=41
			When bill购买zy1的商品::apiserver
				"""
				{
					"order_id": "10101",
					"products": [{
						"name": "商品1a",
						"count": 2
					},{
						"name": "商品1b",
						"count": 1
					}],
					"pay_type":"微信支付",
					"ship_name":"bill",
					"ship_tel":"13811223344",
					"ship_area": "北京市 北京市 海淀区",
					"ship_address": "海淀科技大厦"
				}
				"""
			And bill使用支付方式'微信支付'进行支付::apiserver
			When bill访问zy1的webapp::apiserver
			When bill绑定微众卡::apiserver
				"""
				{
					"binding_date":"2016-06-16",
					"binding_shop":"zy1",
					"weizoom_card_info":
						{
							"id":"100000001",
							"password":"1234567"
						}
				}
				"""
		#10102-优惠抵扣（微众卡全额支付,满额包邮）-商品1a,2+商品2a,1+商品3a,1
			When bill购买zy1的商品::apiserver
				"""
				{
					"order_id": "10102",
					"products": [{
						"name": "商品1a",
						"count": 2
					},{
						"name": "商品2a",
						"count": 1
					},{
						"name": "商品3a",
						"count": 1
					}],
					"weizoom_card":[{
						"card_name":"100000001",
						"card_pass":"1234567"
						}],
					"pay_type":"微信支付",
					"ship_name":"bill",
					"ship_tel":"13811223344",
					"ship_area": "北京市 北京市 海淀区",
					"ship_address": "海淀科技大厦"
				}
				"""
		#10103-微信支付（现金+微众卡，不满足满额包邮）-商品1a,1+商品2a,1+商品3a,1
			When bill购买zy1的商品::apiserver
				"""
				{
					"order_id": "10103",
					"products": [{
						"name": "商品1a",
						"count": 1
					},{
						"name": "商品2a",
						"count": 1
					},{
						"name": "商品3a",
						"count": 1
					}],
					"weizoom_card":[{
						"card_name":"100000001",
						"card_pass":"1234567"
						}],
					"pay_type":"微信支付",
					"ship_name":"bill",
					"ship_tel":"13811223344",
					"ship_area": "北京市 北京市 海淀区",
					"ship_address": "海淀科技大厦"
				}
				"""
			And bill使用支付方式'微信支付'进行支付::apiserver

@mall2 @ztq  @order @allOrder @refund @ztqb
Scenario:1 查看ziying自营平台单个子订单退款录入弹窗详情（现金+微众卡+优惠券）
	#10101-微信支付（商品1a,2+商品1b）-现金21+微众卡10+优惠券10=41
	Given zy1登录系统
	When zy1创建优惠券发放规则发放优惠券
		"""
		{
			"name": "全店通用券1",
			"count": 1,
			"members": ["bill"]
		}
		"""
	When bill访问zy1的webapp::apiserver
	When bill绑定微众卡::apiserver
		"""
		{
			"binding_date":"2016-06-16",
			"binding_shop":"zy1",
			"weizoom_card_info":
				{
					"id":"100000001",
					"password":"1234567"
				}
		}
		"""
	When bill购买zy1的商品::apiserver
		"""
		{
			"order_id": "10101",
			"products": [{
				"name": "商品1a",
				"count": 2
			},{
				"name": "商品1b",
				"count": 1
			}],
			"pay_type":"微信支付",
			"coupon": "coupon1_id_1",
			"weizoom_card":[{
				"card_name":"100000001",
				"card_pass":"1234567"
				}],
			"ship_name":"bill",
			"ship_tel":"13811223344",
			"ship_area": "北京市 北京市 海淀区",
			"ship_address": "海淀科技大厦"
		}
		"""
	And bill使用支付方式'微信支付'进行支付::apiserver
	Given zy1登录系统
	Then zy1获得自营订单'10101-商家1'退款录入详情
		"""
		{
			"need_refund":41.00,
			"origin_order_details":
				{
					"cash":21.00,
					"weizoom_card":10.00,
					"coupon_money":10.00,
					"integral_money":0.00,
					"total":41.00
				},
			"integral_each_yuan":2
		}
		"""
	Given zy1设定会员积分策略
		"""
		{
			"integral_each_yuan":3
		}
		"""
	Then zy1获得自营订单'10101-商家1'退款录入详情
			"""
			{
				"need_refund":41.00,
				"origin_order_details":
					{
						"cash":21.00,
						"weizoom_card":10.00,
						"coupon_money":10.00,
						"integral_money":0.00,
						"total":41.00
					},
				"integral_each_yuan":3
			}
			"""

@mall2 @ztq @order @allOrder @refund
Scenario:2 查看ziying自营平台多个子订单退款录入弹窗详情（现金+微众卡+积分）
	Given zy1登录系统
	When zy1'申请退款'自营订单'10102-商家1'
		"""
		{
			"cash":0.00,
			"weizoom_card":20.00,
			"coupon_money":0.00,
			"integral":0,
			"integral_money":0.00
		}
		"""
	Then zy1获得自营订单'10102'
			"""
			{
				"order_no":"10102",
				"status":"待发货",
				"ship_name":"bill",
				"ship_tel":"13811223344",
				"ship_area": "北京市 北京市 海淀区",
				"ship_address": "海淀科技大厦",
				"invoice":"",
				"business_message":"",
				"methods_of_payment":"优惠抵扣",
				"group":
					[{
						"order_no":"10102-商家1",
						"products":[{
							"name":"商品1a",
							"supplier":"商家1",
							"price":10.00,
							"count":2,
							"single_save":0
							}],
						"postage": 0.00,
						"status":"退款中"
					},{
						"order_no":"10102-商家2",
						"products":[{
							"name":"商品2a",
							"supplier":"商家2",
							"price":10.00,
							"count":1,
							"single_save":0
							}],
						"postage": 0.00,
						"status":"待发货"
					},{
						"order_no":"10102-商家3",
						"products":[{
							"name":"商品3a",
							"supplier":"商家3",
							"price":10.00,
							"count":1,
							"single_save":0
							}],
						"postage": 0.00,
						"status":"待发货"
					}],
				"total_save":"",
				"weizoom_card":40.00,
				"products_count":4,
				"total_price": 40.00,
				"postage": 0.00,
				"cash":0.00,
				"final_price": 40.00
			}
			"""
	#10102-微信支付（商品1a,1+商品2a+商品3a）-现金15+微众卡10+积分抵扣5=30
	Given zy1登录系统
	When zy1创建限时抢购活动
		"""
		[{
			"name": "商品1a限时抢购",
			"promotion_title":"",
			"start_date": "今天",
			"end_date": "1天后",
			"product_name":"商品1a",
			"member_grade": "全部会员",
			"count_per_purchase": 2,
			"promotion_price": 9.00,
			"limit_period": 1
		}]
		"""
	When zy1创建积分应用活动
		"""
		[{
			"name": "积分应用活动1",
			"start_date": "今天",
			"end_date": "1天后",
			"product_name": "商品2a",
			"is_permanant_active": false,
			"rules": [{
			"member_grade": "全部",
			"discount": 50,
			"discount_money": 5.00
			}]
		}]
		"""
	When zy1给"bill"加积分
		"""
		{
			"integral":500,
			"reason":""
		}
		"""
	When bill访问zy1的webapp::apiserver
	When bill绑定微众卡::apiserver
		"""
		{
			"binding_date":"2016-06-16",
			"binding_shop":"zy1",
			"weizoom_card_info":
				{
					"id":"100000002",
					"password":"1234567"
				}
		}
		"""
	When bill购买zy1的商品::apiserver
		"""
		{
			"order_id": "10102",
			"products": [{
				"name": "商品1a",
				"count": 1,
				"postage": 1.00
			},{
				"name": "商品2a",
				"count": 1,
				"postage":0.00
			},{
				"name": "商品3a",
				"count": 1,
				"postage": 0.00,
				"integral": 10,
				"integral_money": 5.00
			}],
			"pay_type":"微信支付",
			"weizoom_card":[{
				"card_name":"100000002",
				"card_pass":"1234567"
				}],
			"ship_name":"bill",
			"ship_tel":"13811223344",
			"ship_area": "北京市 北京市 海淀区",
			"ship_address": "海淀科技大厦"
		}
		"""
	And bill使用支付方式'微信支付'进行支付::apiserver
	Given zy1登录系统
	#未申请退款时查看各个子订单的退款录入弹窗详情
		Then zy1获得自营订单'10102-商家1'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":2
			}
			"""
		And zy1获得自营订单'10102-商家2'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":2
			}
			"""
		And zy1获得自营订单'10102-商家3'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":2
			}
			"""
	#修改积分规则后，查看退款录入弹窗，母订单支付金额的积分抵扣金额应不变
		Given zy1设定会员积分策略
			"""
			{
				"integral_each_yuan":3
			}
			"""
		Then zy1获得自营订单'10102-商家1'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":3
			}
			"""
		And zy1获得自营订单'10102-商家2'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":3
			}
			"""
		And zy1获得自营订单'10102-商家3'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"integral_each_yuan":3
			}
			"""
	#申请退款'子订单1'后查看'子订单2'和'子订单3'的退款录入弹窗详情，增加'已录入退款金额'信息
		#退现金4.00+微众卡2.00+积分抵扣4.00-退款中
		When zy1'申请退款'自营订单'10102-商家1'
			"""
			{
				"cash":4.00,
				"weizoom_card":2.00,
				"coupon_money":0.00,
				"integral":12,
				"integral_money":4.00
			}
			"""
		Then zy1获得自营订单'10102-商家2'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"have_refund_details":
					{
						"cash":4.00,
						"weizoom_card":2.00,
						"coupon_money":0.00,
						"integral_money":4.00,
						"total":10.00
					},
				"integral_each_yuan":3
			}
			"""
		And zy1获得自营订单'10102-商家3'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"have_refund_details":
					{
						"cash":4.00,
						"weizoom_card":2.00,
						"coupon_money":0.00,
						"integral_money":4.00,
						"total":10.00
					},
				"integral_each_yuan":3
			}
			"""
		#修改积分规则，已录入的子订单退款的积分抵扣金额也不会变化
		Given zy1设定会员积分策略
			"""
			{
				"integral_each_yuan":2
			}
			"""
		Then zy1获得自营订单'10102-商家2'退款录入详情
			"""
			{
				"need_refund":10.00,
				"origin_order_details":
					{
						"cash":15.00,
						"weizoom_card":10.00,
						"coupon_money":0.00,
						"integral_money":5.00,
						"total":30.00
					},
				"have_refund_details":
					{
						"cash":4.00,
						"weizoom_card":2.00,
						"coupon_money":0.00,
						"integral_money":4.00,
						"total":10.00
					},
				"integral_each_yuan":2
			}
			"""
	#再申请退款'子订单2'后查看'子订单3'的退款录入弹窗详情，'已录入退款金额'信息会累计显示子订单2的退款金额
		#退现金4.00+微众卡1.00+优惠券4.00+积分抵扣1.00-退款中（退款成功）
			When zy1'申请退款'自营订单'10102-商家2'
				"""
				{
					"cash":4.00,
					"weizoom_card":1.00,
					"coupon_money":4.00,
					"integral":2,
					"integral_money":1.00
				}
				"""
			Then zy1获得自营订单'10102-商家3'退款录入详情
				"""
				{
					"need_refund":10.00,
					"origin_order_details":
						{
							"cash":15.00,
							"weizoom_card":10.00,
							"coupon_money":0.00,
							"integral_money":5.00,
							"total":30.00
						},
					"have_refund_details":
						{
							"cash":8.00,
							"weizoom_card":3.00,
							"coupon_money":4.00,
							"integral_money":5.00,
							"total":20.00
						},
					"integral_each_yuan":2
				}
				"""
			When zy1通过财务审核'退款成功'自营订单'10102-商家2'
			Then zy1获得自营订单'10102-商家3'退款录入详情
				"""
				{
					"need_refund":10.00,
					"origin_order_details":
						{
							"cash":15.00,
							"weizoom_card":10.00,
							"coupon_money":0.00,
							"integral_money":5.00,
							"total":30.00
						},
					"have_refund_details":
						{
							"cash":8.00,
							"weizoom_card":3.00,
							"coupon_money":4.00,
							"integral_money":5.00,
							"total":20.00
						},
					"integral_each_yuan":2
				}
				"""