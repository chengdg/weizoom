...
"""
	...
	备注：
		...
		打印快递单检查点：
			1、选择需要打印的一个或多个订单后，点击“打印快递单”，出现弹窗，可选择快递模板，快递目前只支持部分：四通一达（申通、圆通、中通、百世汇通、韵达）、顺丰、EMS、德邦
			2、如果未提前设置发件人信息，打印快递单时，可弹层提示“请先设置发货人信息”；如果已有默认地址，打印快递单时，直接使用默认地址
			3、打印快递单时，若收货人信息相同（收货人姓名、收货人电话、收货人地址相同），合并打印一张发货单
			4、打印快递单时，订单自动标记为已发货，且自动填充物流信息

		添加新地址表单检查点：
			1、表单包含：发货人（必填）、所在地区（必填）、详细地址（必填）、邮政编码、发货人电话（必填）、公司名称、备注
			2、发货人电话有位数、号段等校验
