...
"""
	1、有效商家是指：
		...
		(2)商家由于过了有效期,或者被强制"关闭",从而从有效商家变为无效商家,之前同步到自营平台的商品,被直接自动删除,如果当前商品在自营平台参与活动,活动结束（多商品券除外）,钉钉提示该商品因为商家失效被删除
...
Scenario:8 自营平台同步商品参与积分应用或优惠券活动,商家更新被自营平台同步商品的【商品重量】【商品图片】
	...
	#jobs自营平台同步商品池中的商品
		...
		Given jobs已添加了优惠券规则
			...
			[{
				...
				"coupon_product": "jobs修改-tom无规格商品1,bill无规格商品1"
			}]
			"""
		Then jobs能获得优惠券规则列表
			"""
	...
	#更新【商品重量】【商品图片】触发更新
		...
		#同步到自营平台的商品，由于商家下架商品，导致自营平台商品被自动删除，
		#对应参与的多商品券全部商品都处于删除状态，多商品券活动自动失效
		Given tom登录系统
		When tom'下架'商品'tom无规格商品1' 
