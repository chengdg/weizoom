...
Feature: 自营平台待售商品列表页
	...
	待售商品列表页：
		...
		13.在商品池更新之后，(填写采购价的商品保留更新之前设置的数值，库存保留之前数据)，别的修改项更新之后被覆盖
...
Scenario:1 自营平台把商品从商品池放入待售商品列表，获取待售商品列表
	...
	#自营平台nokia登录
	Given nokia登录系统
	When nokia批量将商品放入待售于'2015-08-02 12:30'
		"""
		[{
			"name": "tom无规格商品3"
		}, {
...
Scenario:3 自营平台把商品从商品池放入待售商品列表后，商户(供货商)修改商品
	...
	Then jobs获得商品池商品列表
		...
		"""
	When jobs更新商品池商品'bill无规格商品1'于'2015-08-03 10:30'
	Then jobs获得商品池商品列表
		"""
		[{
			"name": "bill商品1",
	...
	#同步时间-以每次更新的时间为主,(填写采购价的商品保留更新之前设置的数值，库存保留之前数据)，别的修改项更新之后被覆盖
	Then jobs能获得'待售'商品列表
		"""
		[{
			"name": "tom无规格商品3",
		...
		"""
	#商户更新商品，商品池跟新后(商品名称，分组，商品编码都被覆盖了)，库存采和购价保留之前数据
	Then jobs能获取商品'tom无规格商品3'
		"""
		{
			"name": "tom无规格商品3",
