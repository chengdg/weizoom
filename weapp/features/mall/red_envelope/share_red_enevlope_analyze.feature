...
Scenario: 1 分享红包分析
	...
	Then jobs能获得分享红包"红包1"的分析详情
		...
		| tom      | 普通会员 | 0            | 0            | 0          | 0          | 今天     | 未使用   | 查看引入详情 |
		| bill     | 普通会员 | 2            | 1            | 0          | 50.00      | 今天     | 未使用   | 查看引入详情 |

	Then jobs能获得分享红包"红包1-bill"的引入详情
		| 分享会员 | 会员状态 | 引入领取人数 | 引入使用人数 | 引入新关注 | 引入消费额 | 领取时间 | 使用状态 | 操作         |
		| nokia    | 非会员   | 0            | 0            | 0          | 0          | 今天     | 未使用   |              |
		| tom      | 普通会员 | 0            | 0            | 0          | 0          | 今天     | 已使用   | 查看使用订单 |
