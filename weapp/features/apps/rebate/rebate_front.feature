...
Scenario:3 带参数返利活动[扫码后成交金额]-已关注会员可参与；首单；非现金
	...
	When 微信用户批量消费jobs的商品
		...
		|   0003   | 今天        |   tom    | 商品1,1 |   支付  |  支付宝   |   0.00    | 1.00    |     1.00        | 0000041,1234567| jobs,完成  |    已完成     |
		|   0004   | 三天后      |  zhouxun | 商品1,1 |   支付  |  微众卡   |   0.00    | 1.00    |     1.00        |                | jobs,完成  |    已完成     |
...
Scenario:4 带参数返利活动[扫码后成交金额]-已关注会员可参与；不限；现金
	...
	When 微信用户批量消费jobs的商品
		...
		|   0003   | 明天       |  zhouxun | 商品1,1 |   支付  |  微众卡   |   0.00   | 1.00   |     1.00       | 0000041,1234567| jobs,完成  |    已完成     |
...
Scenario:8 带参数返利活动[扫码后成交金额]-已关注会员不可参与；首单；现金
	...
	When 微信用户批量消费jobs的商品
		| order_id | date       | consumer | product | payment | pay_type  |postage*  |price*  | paid_amount*   | weizoom_card   | action     | order_status  |
		|   0001   | 两天前     |   bill   | 商品1,1 |   支付  |  支付宝   |   0.00   | 0.00   |     1.00       |                | jobs,完成  |    已完成     |
...
Scenario:9 带参数返利活动[扫码后成交金额]-已关注会员不可参与；首单；非现金
	...
	When 微信用户批量消费jobs的商品
		...
		|   0005   | 明天       |  zhouxun | 商品1,1 |   支付  |  微众卡   |   0.00   | 1.00   |     1.00       | 0000041,1234567| jobs,完成     |    已完成     |
		|   0006   | 明天       |  zhouxun | 商品1,1 |   支付  |  微众卡   |   0.00   | 1.00   |     1.00       | 0000041,1234567| jobs,完成     |    已完成     |
...
Scenario:11 带参数返利活动-查看发放详情
	...
	When bill购买jobs的商品
		| order_id | date         | consumer | product | payment | pay_type  |postage*   |price*   | paid_amount*    | weizoom_card    | action     | order_status  |
		|   0001   | 三天后       |   bill   | 商品1,1 |   支付  |  支付宝   |   0.00    | 1.00    |     1.00        | 0000001,1234567 | jobs,完成  |    已完成     |
