...
Scenario:3 带参数返利活动[扫码后成交金额]-已关注会员可参与；首单；非现金
	...
	#显示所有的订单
	When jobs取消对'返利活动2'进行'仅显示扫码后成交订单'操作
	Then jobs能获取'返利活动2'订单列表
	"""
		[{
			"order_id":"0001",
...
Scenario:9 带参数返利活动[扫码后成交金额]-已关注会员不可参与；首单；非现金
	...
	When 微信用户批量消费jobs的商品
		...
		|   0002   | 今天       |   bill   | 商品1,1 |        |  支付宝   |   0.00   | 1.00   |     0          |                |              |    待支付     |
		...
		|   0006   | 明天       |  -zhouxun | 商品1,1 |   支付  |  支付宝   |   0.00   | 1.00   |     1.00       |                | jobs,完成     |    已完成     |
	...
	Given jobs登录系统
	When jobs'申请退款'订单'0006'
	When jobs通过财务审核'退款成功'订单'0006'
	#默认显示勾选仅显示扫码后的成交的订单
