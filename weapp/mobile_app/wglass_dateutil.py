...
def get_month_range(month_before):
	date_item = []
	d = datetime.now()
	for m in range(0, month_before):
		date_item.append(d.strftime('%Y-%m'))
		d = (d.replace(day=1) - timedelta(1)).replace(day=1)
	date_item.sort()
	
	return date_item


def get_month_before(month_before):
	d = datetime.now()
	for m in range(1, month_before):
		d = (d.replace(day=1) - timedelta(1)).replace(day=1)
	
	return d


def get_date_range(date_range=None, month_before=6):
	if date_range and len(date_range) > 0:
		date_items = date_range.split(' - ')
		start_date = date_items[0] + ' 00:00:00'
		if len(date_items) == 1:
			end_date = date_items[0] + ' 23:59:59'
		else:
			end_date = date_items[1] + ' 23:59:59'
	else:
		start_date = get_month_before(month_before).strftime('%Y-%m-%d 00:00:00')
