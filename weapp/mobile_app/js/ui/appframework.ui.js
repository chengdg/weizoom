    ...
    var scroller = (function () {
        ...
        scrollerCore.prototype = {
            ...
            fireRefreshRelease: function (triggered) {
	            ...
	            //修复还未出现‘释放刷新’就会刷新的bug ----start
                if(!this.refreshTriggered){
                    this.preventHideRefresh = false;
                    this.hideRefresh();
                    return;
                }
                //修复还未出现‘释放刷新’就会刷新的bug ----end
        ...
        jsScroller.prototype.onTouchEnd = function (event) {
			...
			var timingFunction_aix = "cubic-bezier(0.33,0.66,0.66,1)";
            //refresh hang in
            if (this.refresh && triggered) {
	            ...
	            timingFunction_aix = "adjustScroll";//设置此标志，以避免scroll 双倍距离的问题
            }
            if(this.elementInfo.hasHorScroll){
                if(scrollInfo.x>=0)
                {
            ...
            this.scrollerMoveCSS(scrollInfo, scrollInfo.duration, timingFunction_aix);
	        if(timingFunction_aix !== "adjustScroll"){
	    ...
	    //修复消息内容被回复框遮盖的问题
        jsScroller.prototype.adjustScrollToBottom = function () {
            var $message_history_el = $(this.el).find('#message_history');
            var opts = {};
            if($message_history_el[0].scrollHeight <= this.container.clientHeight){
                opts = {x:0,y:0};
            }else{
                opts = {
                    y: this.container.clientHeight - $message_history_el[0].scrollHeight-45,
                    x: 0
                };
            }
            this.scrollTo(opts,40);
        };
            ...
            if (this.el && this.el.style) {
                ...
                if (this.eventsActive) {
                    if (this.androidFormsMode) {
                        ...
                        if(timingFunction !== "adjustScroll") $(this.el).animate(opts).start();
                    }
                }
