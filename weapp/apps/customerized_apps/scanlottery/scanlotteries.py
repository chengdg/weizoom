...
class Scanlotteries(resource.Resource):
	app = 'apps/scanlottery'
	resource = 'scanlotteries'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.Scanlottery.objects.count()
		...
		return render_to_response('scanlottery/templates/editor/scanlotteries.html', c)
	...
	def get_datas(request):
		...
		datas_datas = app_models.Scanlottery.objects(**params)
		...
		datas = app_models.Scanlottery.objects(**params).order_by('-id')
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = Scanlotteries.get_datas(request)
