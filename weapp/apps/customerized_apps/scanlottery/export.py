# -*- coding: utf-8 -*-
from apps.customerized_apps.scanlottery import scanlotteries
...
def get_link_targets(request):
	...
	pageinfo, datas = scanlotteries.Scanlotteries.get_datas(request)
	link_targets = []
	for data in datas:
		link_targets.append({
			"id": str(data.id),
			"name": data.name,
			"link": '/m/apps/scanlottery/m_scanlottery_page/?webapp_owner_id=%d&id=%s' % (request.manager.id, data.id),
