		...
		if 'new_app:' in record_id:
			...
			if record.count() > 0:
				...
				if fid is None or str(fid) == str(member_id):
					...
					user_id = request.webapp_owner_info.user_profile.user_id
					username = User.objects.get(id=user_id).username
					has_power = True if username == 'weshop' and (app_models.PoweredLimitRelation.objects(belong_to=record_id,member_id=member_id,powered_member_id=fid).count() > 0 or app_models.PoweredDetail.objects(belong_to=record_id, power_member_id=member_id).count() > 0) else False
