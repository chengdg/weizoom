...
class weizoom_eventParticipances(resource.Resource):
	app = 'apps/weizoom_event'
	resource = 'weizoom_event_participances'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.weizoom_eventParticipance.objects(belong_to=request.GET['id']).count()
		...
		return render_to_response('weizoom_event/templates/editor/weizoom_event_participances.html', c)
	...
	def get_datas(request):
		...
		datas = app_models.weizoom_eventParticipance.objects(**params).order_by('-id')	
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = weizoom_eventParticipances.get_datas(request)
