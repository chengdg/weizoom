...
class test1Participances(resource.Resource):
	app = 'apps/test1'
	resource = 'test1_participances'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.test1Participance.objects(belong_to=request.GET['id']).count()
		...
		return render_to_response('test1/templates/editor/test1_participances.html', c)
	...
	def get_datas(request):
		...
		datas = app_models.test1Participance.objects(**params).order_by('-id')	
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = test1Participances.get_datas(request)
