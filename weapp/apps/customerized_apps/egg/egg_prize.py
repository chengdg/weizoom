...
class egg_prize(resource.Resource):
	app = 'apps/egg'
	resource = 'egg_prize'
	...
	def api_post(request):
		...
		egg_id = request.POST['id']
		member_id = request.member.id
		try:
			if tel:
				latest_record = app_models.EggRecord.objects(member_id=member_id, belong_to=egg_id).first()
	...
	def api_put(request):
		...
		egg_participances = app_models.EggParticipance.objects(belong_to=record_id, member_id=member_id)
		if egg_participances.count() != 0:
			egg_participance = egg_participances.first()
		else:
			#如果当前用户没有参与过该活动，则创建新记录
			data['belong_to'] = record_id
			data['egg_date'] = now_datetime - dt.timedelta(seconds=2) #第一次参与抽奖初始抽奖时间向前2秒以免在下面的逻辑中被判定为过于频繁
		...
		if not egg_prize:
			...
			if egg_prize_type == 'coupon':
				...
				coupon, msg, _ = get_consume_coupon(egg.owner_id, 'egg',str(egg.id), egg_prize_data, member_id)
				if not coupon:
					result = u'谢谢参与'
					egg_prize_type = 'no_prize'
				else:
