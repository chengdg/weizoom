...
class EggParticipances(resource.Resource):
	app = 'apps/egg'
	resource = 'egg_participances'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.EggParticipance.objects(belong_to=request.GET['id']).count()
		...
		return render_to_response('egg/templates/editor/egg_participances.html', c)
	...
	def get_datas(request):
		...
		# datas = app_models.eggParticipance.objects(**params).order_by('-id')
		datas = app_models.EggRecord.objects(**params)
		if not export_id:
			#进行分页
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = EggParticipances.get_datas(request)
	...
	def api_post(request):
		...
		领取奖品
		"""
		app_models.EggRecord.objects(id=request.POST['id']).update(set__status=True)
		response = create_response(200)
		return response.get_response()

class EggParticipances_Export(resource.Resource):
	'''
	批量导出
	'''
	app = 'apps/egg'
	resource = 'egg_participances-export'
	@login_required
	def api_get(request):
		"""
		详情导出
		...
		# app_name = eggParticipances_Export.app.split('/')[1]
		# excel_file_name = ('%s_id%s_%s.xls') % (app_name,export_id,datetime.now().strftime('%Y%m%d%H%m%M%S'))
		download_excel_file_name = u'砸金蛋详情.xls'
		excel_file_name = 'egg_details_'+datetime.now().strftime('%H_%M_%S')+'.xls'
		...
		try:
			import xlwt
			datas = EggParticipances.get_datas(request)
			fields_raw = []
			export_data = []
