...
class EggParticipance(resource.Resource):
	app = 'apps/egg'
	...
	def api_get(request):
		...
		if 'id' in request.GET:
			egg_participance = app_models.EggParticipance.objects.get(id=request.GET['id'])
			data = egg_participance.to_json()
		else:
			data = {}
	...
	def api_put(request):
		...
		#调整参与数量
		app_models.Egg.objects(id=data['belong_to']).update(**{"inc__participant_count":1})
		...
		if prize:
			...
			if prize_type == 'no_prize':
				...
				if not request.member:
					...
					coupon, msg, _ = get_consume_coupon(request.webapp_owner_id, 'egg', data['belong_to'], coupon_rule_id, request.member.id)
					if not coupon:
						error_msg = msg
		
		data = json.loads(egg_participance.to_json())
