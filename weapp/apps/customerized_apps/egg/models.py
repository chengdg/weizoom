...
class EggParticipance(models.Document):
	...
	egg_date = models.DateTimeField() #最近一次砸金蛋时间

	meta = {
		'collection': 'egg_egg_participance',
		'db_alias': 'apps'
	}

class EggRecord(models.Document):
	"""
	砸金蛋记录表
	"""
	webapp_user_id = models.LongField(default=0) #参与者id
	...
	egg_name = models.StringField(default="", max_length=100) #对应的砸金蛋活动title
...
STATUS_RUNNING = 1
STATUS_STOPED = 2
class Egg(models.Document):
	owner_id = models.LongField() #创建人id
	name = models.StringField(default="", max_length=100) #名称
	egg_type = models.StringField(default="roulette", max_length=100) #砸金蛋类型
	...
	@property
	def egg_type_cn(self):
		return (u'大转盘', u'刮刮乐', u'红包')[('roulette', 'scratch', 'red').index(self.egg_type)]
...
class EggControl(models.Document):
	belong_to = models.StringField(default="", max_length=100) #对应的砸金蛋活动id
	member_id= models.LongField(default=0) #参与者id
