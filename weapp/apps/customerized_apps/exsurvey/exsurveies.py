...
class exsurveies(resource.Resource):
	app = 'apps/exsurvey'
	resource = 'exsurveies'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.exsurvey.objects.count()
		...
		return render_to_response('exsurvey/templates/editor/exsurveies.html', c)
	...
	def get_datas(request):
		...
		datas_datas = app_models.exsurvey.objects(**params)
		...
		if prize_type != 'all':
			records = []
			prize_type_ids = []
			records = app_models.exsurvey.objects.filter(owner_id=request.manager.id)
		...
		datas = app_models.exsurvey.objects(**params).order_by('-id')
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = exsurveies.get_datas(request)
