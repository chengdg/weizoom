# -*- coding: utf-8 -*-
from apps.customerized_apps.exsurvey import exsurveies
...
def get_link_targets(request):
	...
	pageinfo, datas = exsurveies.exsurveies.get_datas(request)
	link_targets = []
	for data in datas:
		link_targets.append({
			"id": str(data.id),
			"name": data.name,
			"link": '/m/apps/exsurvey/m_exsurvey/?webapp_owner_id=%d&id=%s' % (request.manager.id, data.id),
