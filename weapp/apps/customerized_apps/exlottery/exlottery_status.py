...
class ExlotteryStatus(resource.Resource):
	...
	def api_get(request):
		...
		响应PGET
		"""
		pageinfo, exlottery_codes, member_id2member_name, count, created_at, status = ExlotteryStatus.get_datas(request)
...
class ExlotteryStatusExport(resource.Resource):
	app = 'apps/exlottery'
	resource = 'exlottery_status_export'
	...
	def api_get(request):
		...
		@return:
		"""
		exlottery_codes, member_id2member_name, count, created_at, status = ExlotteryStatus.get_datas(request)
