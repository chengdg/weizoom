...
"""
验证码
代码来源：wheezy.captcha.image
修改原因：验证码显示不全，源码缺陷导致
修改行数：107
修改详情：c_height 改为 c_height+10
修改2：旋转、缩放 修改背景色为白色（黑色背景框暂时没有研究如何去掉）
"""

import random

from wheezy.captcha.comp import Draw, Image, ImageFilter, getrgb, truetype, xrange
...
def text(fonts, font_sizes=None, drawings=None, color='#5C87B2',
    ...
    def drawer(image, text):
        ...
        for c in text:
            ...
            char_image = Image.new('RGB', (c_width, c_height+10), (255,255,255))
...
def warp(dx_factor=0.27, dy_factor=0.21):
    def drawer(image):
        ...
        image2 = Image.new('RGB',
                           (width + abs(x1) + abs(x2),
                            height + abs(y1) + abs(y2)), (255,255,255))
...
def offset(dx_factor=0.1, dy_factor=0.2):
    def drawer(image):
        ...
        image2 = Image.new('RGB', (width + dx, height + dy), (255,255,255))
