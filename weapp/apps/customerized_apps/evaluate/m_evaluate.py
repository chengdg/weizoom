	...
	def get(request):
		...
		if 'id' in request.GET:
			...
			if 'new_app:' in id:
				...
				#termite类型数据
				try:
					record = app_models.Evaluate.objects.get(id=id)
				except:
				...
				project_id = 'new_app:evaluate:%s' % record.related_page_id
				
				if request.member:
					participance_data_count = app_models.EvaluateParticipance.objects(belong_to=id, member_id=request.member.id).count()
