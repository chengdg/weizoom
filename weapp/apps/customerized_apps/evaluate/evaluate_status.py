...
class EvaluateStatus(resource.Resource):
	app = 'apps/evaluate'
	resource = 'evaluate_status'
	...
	def api_post(request):
		...
		if target_status == 'stoped':
			...
			app_models.Evaluate.objects(id=request.POST['id']).update(set__end_time=now_time)
			pagestore = pagestore_manager.get_pagestore('mongo')
			datas = app_models.Evaluate.objects(id=request.POST['id'])
			for data in datas:
		...
		app_models.Evaluate.objects(id=request.POST['id']).update(set__status=target_status)
