...
def pay(pay_interface, order, webapp_owner_id):
	if PAY_INTERFACE_ALIPAY == pay_interface.type:
		...
		return './?woid={}&module=apps:shihuazhiye:mall&model=pay_result&action=get&pay_interface_type={}&order_id={}'.format(webapp_owner_id, PAY_INTERFACE_COD, order.order_id)
	elif PAY_INTERFACE_WEIXIN_PAY == pay_interface.type:
		return '/webapp/wxpay/?woid={}&order_id={}&pay_id={}&callback_module=apps:shihuazhiye:mall&showwxpaytitle=1'.format(webapp_owner_id, order.order_id, pay_interface.id)
