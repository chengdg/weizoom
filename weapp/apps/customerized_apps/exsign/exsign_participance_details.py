...
class exSignParticipances(resource.Resource):
	app = 'apps/exsign'
	resource = 'exsign_participance_details'
	...
	def api_get(request):
		...
		if member_id and belong_to:
			...
			items = app_models.exSignDetails.objects(belong_to=belong_to, member_id=int(member_id),created_at__gte=start_date,created_at__lte=end_date).order_by('-created_at')
