...
class ShengjingGetCaptcha(object):
	'''
	...
	def _captcha_response(self):
		"""使用MPhone获取captcha"""
		param_json_data = {
			self.shengjing_params.METHOD: self.shengjing_config.method_get_captcha,
		...
		try:
			...
			watchdog_info(u"get_captcha_data url: %s ,"
				u"param_data: %s" % (self.api_url, param_data), self.shengjing_params.WATCHDOG_TYPE_SHENGJING)
		except:
			watchdog_error(u'从Shengjing获取get captcha失败，url:{},data:{},原因:{}'.format(self.api_url,
				param_data,
				unicode_full_stack()))
			notify_message = u'从Shengjing获取get captcha失败，url:{},data:{},原因:{}'.format(self.api_url,
				param_data,
				unicode_full_stack())
	...
	def _parse_result(self, result):
		...
		try:
			...
			notify_message = u'shengjing解析get captcha失败，url:{},原因:{}'.format(result, unicode_full_stack())
			watchdog_fatal(notify_message, self.shengjing_params.WATCHDOG_TYPE_SHENGJING)
	...
	def get_captcha_data(self):
		...
		watchdog_info(u"盛景获取验证码 get_captcha_data \n url:\n{}?{}\n data:\n{}".format(
