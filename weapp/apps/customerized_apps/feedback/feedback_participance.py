...
class feedbackParticipance(resource.Resource):
	app = 'apps/feedback'
	...
	def api_get(request):
		...
		if 'id' in request.GET:
			feedback_participance = app_models.feedbackParticipance.objects.get(id=request.GET['id'])
			data = feedback_participance.to_json()
		else:
			data = {}
	...
	def api_put(request):
		...
		feedback_participance = app_models.feedbackParticipance(**data)
		feedback_participance.save()
		error_msg = None
		...
		data = json.loads(feedback_participance.to_json())
