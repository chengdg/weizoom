...
class feedbackParticipances(resource.Resource):
	app = 'apps/feedback'
	resource = 'feedback_participances'
	...
	def get(request):
		...
		响应GET
		"""
		has_data = app_models.feedbackParticipance.objects(belong_to=request.GET['id']).count()
		...
		return render_to_response('feedback/templates/editor/feedback_participances.html', c)
	...
	def api_get(request):
		...
		响应API GET
		"""
		pageinfo, datas = feedbackParticipances.get_datas(request)
