...
W.view.weixin.EmbededPhoneMenuBar = Backbone.View.extend({
    ...
    getMenu: function(menuId) {
    	return this.weixinMenu.getMenu(menuId);
    },
    
    getMenuItem: function(menuId, itemId) {
    	return this.weixinMenu.getMenuItem(menuId, itemId);
    },
    ...
    showMenuItems: function(menuId) {
        ...
        //显示menu item
        this.$menuItemContainer.css('left', popoverLeft + 18);
...
W.view.weixin.EmbededPhoneView = Backbone.View.extend({
    ...
    compileTemplate: function() {
        ...
        $('#weixin-material-single-news-tmpl-src').template('weixin-material-single-news-tmpl');
        $('#weixin-material-multi-newses-tmpl-src').template('weixin-material-multi-newses-tmpl');
        $('#weixin-material-text-message-tmpl-src').template('weixin-material-text-message-tmpl');
    },
...
//			return message.type == 'news' && message.id < 0;
    ...
    onDeleteNews: function(event){
        ...
        this.enableAddNews = this.messages.length <= 9 ? true : false;
    ...
    validate: function(){
        var selectedNewsId = this.selectedNewsId;
        if (this.onBeforeChangeNews) {
            for(var i=0; i<this.messages.models.length; i++) {
                var message = this.messages.models[i];
                this.selectNews(parseInt(message.id));
                if (!this.onBeforeChangeNews()) {
                    return false;
                }
            }
