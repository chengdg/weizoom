...
class WoFuHandler(KeywordHandler):
	...
	def _handle_keyword(self, context, from_weixin_user, is_from_simulator):
		...
		if message.msgType == WeixinMessageTypes.TEXT and user_profile.user_id in [,570]:
						...
						if WoFuLog.objects.filter(member_id=member.id).count() == 0:
							...
							with transaction.atomic():
								...
								if wo_fu.is_send is False:
									...
									response_content = u'老朋友决不能冒名取代，每一个老朋友对于窝夫来说都是独一无二的，此手机号已经领取过相应优惠券，别犹豫，让真实的你成为窝夫小子真正的朋友。'
						else:
							response_content = u'老朋友决不能冒名取代，每一个老朋友对于窝夫来说都是独一无二的，此手机号已经领取过相应优惠券，别犹豫，让真实的你成为窝夫小子真正的朋友。'
					else:
						response_content = u'我们的故事还没有开启，最甜美的故事脚本快点儿翻开吧，窝夫小子上架新宠“一盒甜品”现在尝鲜价，就用它来开始我们的故事吧，欢迎选购，让我们成为分不开的朋友。'
