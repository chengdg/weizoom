...
class AutoQaMessageHandler(KeywordHandler):
	...
	def _handle_keyword(self, context, from_weixin_user, is_from_simulator):
		...
		response_rule = cache_util.find_answer_from_cache_for(user_profile.webapp_id, message.content.lower())
	...
	def _process_recorde_message(self, context, response_rule, from_weixin_user, is_from_simulator):
		...
		from weixin.message.qa.tasks import recorde_message, _recorde_message
	...
	def _build_response_without_auto_response_rule(self, user_profile, message, is_from_simulator):
		...
		if response_rule and response_rule.is_active:
			token = self._get_token_for_weixin_user(user_profile, message.fromUserName, is_from_simulator)
			if response_rule.material_id > 0:
				response = generator.get_news_response(message.fromUserName, message.toUserName, response_rule.newses, token)
			else:
				if is_from_simulator:
					...
					response = generator.get_text_response(message.fromUserName, message.toUserName, response_rule.answer, token, user_profile)
