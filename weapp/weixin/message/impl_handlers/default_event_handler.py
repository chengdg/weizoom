...
class DefaultEventHandler(EventMessageHandler):
	...
	def _move_weixin_followers_statistics_one_step(self, user, is_from_simulator, is_increse=True):
	...
	def _handle_subscribe_event(self, context, is_from_simulator):
		...
		try:
			...
			self._move_weixin_followers_statistics_one_step(user_profile.user, is_from_simulator)
			## weshop begin
	...
	def _handle_unsubscribe_envent(self, context, is_from_simulator):
		...
		try:
			...
			self._move_weixin_followers_statistics_one_step(user_profile.user, is_from_simulator, False)
			return None
